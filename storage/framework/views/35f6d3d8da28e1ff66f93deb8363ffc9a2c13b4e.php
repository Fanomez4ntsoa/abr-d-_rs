<?php $__currentLoopData = $shorts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $short): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="video-shorts n_video_short shorts-fixed-hight video-poster card single-item-countable" id="shorts-<?php echo e($short->id); ?>">
            <div class="position-relative shorts-height">
                ...
                <div class="video-meta short_v_con w-100 rounded-3" onclick="videoPlaytoggle('<?php echo e('#shorts_'.$short->id); ?>')">
                    <div class="video-avatar custom-shorts-heading">
                        ...
                        <div class="avatar-body d-flex align-items-center justify-content-between">
                            ...
                            ?>
                            <?php if($follow>0): ?>
                                <a href="javascript:void(0)" onclick="event.stopPropagation(); ajaxAction('<?php echo route('user.unfollow',$short->getUser->id); ?>')" class="btn common_btn_2"><?php echo e(get_phrase('Unfollow')); ?></a> 
                            <?php else: ?>
                                <a href="javascript:void(0)" onclick="event.stopPropagation(); ajaxAction('<?php echo route('user.follow',$short->getUser->id); ?>')" class="btn common_btn"><?php echo e(get_phrase('Follow')); ?></a> 
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php
                    ...
                    <div class="video-share  fs-4">
                        ...
                        <span class="entry-react post-react eFont custom-text-shadow">
                            <a href="#" onclick="event.stopPropagation(); myReact('post', 'like', 'toggle', <?php echo e($post->post_id); ?>, 'number')" id="reactNumber<?php echo $post->post_id; ?>">
                                ...
                                <span class="fs-6 custom-text-shadow appendNumber"> <?php echo e($totalreact); ?></span>
                            </a>
                        </span>
                        <a href="#" onclick="event.stopPropagation();" data-bs-toggle="modal" data-bs-target="#ShortChat<?php echo e($short->id); ?>"> <i class="fa-solid fa-comment custom-texts"></i> <br> <span class="fs-6"><?php echo e($total_comments); ?></span></a>
                        <a href="#" onclick="event.stopPropagation(); showCustomModal('<?php echo e(route('load_modal_content', ['view_path' => 'frontend.main_content.share_post_modal', 'post_id' => $post->post_id] )); ?>', '<?php echo e(get_phrase('Share post')); ?>');"> <i class="fa-solid fa-share custom-texts"></i><span class="fs-6 custom-text-shadow"><?php echo e(get_phrase('Share')); ?></span></a>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="user-comments bg-white" id="user-comments-<?php echo e($post->post_id); ?>" >
                            <div class="comment-form d-flex p-3 bg-secondary">
                                <img src="<?php echo e(get_user_image(Auth()->user()->photo, 'optimized')); ?>" alt="" class="rounded-circle img-fluid h-39" width="40px">
                                <form action="javascript:void(0)" class="w-100 ms-2" method="post">
                                    <input class="form-control py-3" onkeypress="postComment(this, 0, <?php echo e($post->post_id); ?>, 0,'post');" rows="1" placeholder="Écrire des commentaires">
                                </form>
                            </div>
                            <ul class="comment-wrap p-3 pb-0 list-unstyled eList" id="comments<?php echo e($post->post_id); ?>">
                            ...
                            <?php if($comments->count() < $total_comments): ?>
                                <a class="btn eColor p-3 pt-0" onclick="loadMoreComments(this, <?php echo e($post->post_id); ?>, 0, <?php echo e($total_comments); ?>,'post')"><?php echo e(get_phrase('View more')); ?></a>
                            <?php endif; ?>
                        </div>
                    </div>                              
                 </div>
...
<?php echo $__env->make('frontend.initialize', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    <?php /**PATH C:\Users\USER\Downloads\Compressed\Piscine de Romain Avril\resources\views/frontend/video-shorts/shorts-single.blade.php ENDPATH**/ ?>