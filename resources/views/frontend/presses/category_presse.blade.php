<div class="page-wrap">
    <div class="d-flex pagetab-head nblog_header p-20 radius-8  align-items-center justify-content-between  bg-white">
        <h3 class="h5 mb-0"> {{ get_phrase('Presses') }}</h3>
        <div class="inline-btn w-50">
            <a href="{{ route('create.presse') }}" class="btn common_btn"><i class="fa fa-plus-circle me-1"></i>{{ get_phrase('Create Presse') }}</a>
        </div>
    </div>
    ...
    <div class="card blog-tags border-none radius-8 mt-12">
        <div class="tags">
            @foreach ($categories as $category )
                <a href="{{ route('category.presse',$category->id) }}" class="@if($category['id'] == $category_id) active @endif">{{ $category->name }}</a>
            @endforeach 
        </div>
    </div>
    ...
    <div class="bg-white radius-8 p-20 Eblogs mt-12">
        ...
        <div class="row g-3 blog-cards">
            @foreach ($presses as $presse )
                <div class="col-lg-4" id="presse-{{ $presse->id }}">
                    <article class="single-entry sblog_entry p-0">
                        <div class="entry-img">
                            <a href="{{ route('single.presse',$presse->id) }}"><img src="{{ get_presse_image($presse->thumbnail,'thumbnail') }}" alt="" class="img-fluid thumbnail-210-200"></a>
                            <span class="date-meta">{{ $presse->created_at->format("d-M-Y") }}</span>
                        </div>
                        <div class="entry-txt p-8">
                            <div class="blog-meta">
                                <span><a href="#">{{ $presse->cagtegory->name }}</a></span>
                            </div>
