<div class="page-wrap">
    <div class="d-flex nblog_header pagetab-head radius-8 align-items-center justify-content-between p-20 bg-white">
        <h3 class="h5 mb-0"> {{ get_phrase('My Presse') }}</h3>
        <div class="">
            <div class="btn-inline ">
                <a href="{{ route('create.presse') }}" class="btn common_btn btn-sm"> <i class="fa fa-circle-plus me-2"></i>{{ get_phrase('Create Presse') }}</a>
                <a href="{{ route('mypresse') }}" class="btn  btn-sm ms-2 @if(Route::currentRouteName() == 'mypresse') common_btn_2 @else common_btn @endif ">{{ get_phrase('My Presse') }}</a>
            </div>
        </div>
    </div>
    <div class="bg-white radius-8 p-20 Eblogs mt-12">
        ...
        <div class="row g-3 blog-cards">
            @foreach ($presses as $presse )
                <div class="col-12 col-sm-6 col-md-4 col-lg-6 col-xl-4" id="presse-{{ $presse->id }}">
                    <article class="single-entry sblog_entry p-0">
                        <div class="entry-img">
                            <a href="{{ route('single.presse',$presse->id) }}"><img src="{{ get_presse_image($presse->thumbnail,'thumbnail') }}" alt="" class="img-fluid w-200 thumbnail-210-200"></a>
                            <span class="date-meta">{{ $presse->created_at->format("d-M-Y") }}</span>
                        </div>
                        <div class="entry-txt p-8">
                            <div class="blog-meta">
                                <span><a href="#">{{ $presse->category->name }}</a></span>
                            </div>
                            <h3 class="h6"><a href="{{ route('single.presse',$presse->id) }}">{{$presse->title}}</a></h3>
                            <div class="d-flex justify-content-between blog-ava">
                                <div class="d-flex">
                                    <img src="{{ get_user_image($presse->user_id,'optimized') }}" class="user-round" alt="">
                                    <div class="ava-info">
                                        <h6><a href="#">{{ $presse->user->name }}</a></h6>
                                ...
                                <div class="dropdown eg_drop bg_drop">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                            <li>
                                                <a href="{{ route('presse.edit',$presse->id) }}" class="dropdown-item btn btn-primary btn-sm"> <i class="fa fa-edit"></i> {{ get_phrase('Edit Article') }}</a>
                                            </li>
                                            <li>
                                                <a href="javascript:void(0)" onclick="confirmAction('<?php echo route('presse.delete', ['presse_id' => $presse->id]); ?>', true)" class="dropdown-item btn btn-primary btn-sm"><i class="fa fa-trash me-1"></i> {{get_phrase('Delete Article')}}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
