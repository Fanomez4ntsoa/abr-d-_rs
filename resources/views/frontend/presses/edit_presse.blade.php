...
<div class="page-wrap">
    <div class="d-flex pagetab-head  align-items-center justify-content-between mb-3 p-20 radius-8 bg-white">
        <h3 class="h5 pt-0 mb-0"><span><img width="12" src="{{ asset('assets/frontend/images/stickies-fill.png') }}" alt=""></span> {{ get_phrase('Edit Presse') }}</h3>
        ...
        <div class="create-article ">
            ...
            <form action="{{ route('presse.update',$presse->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" value="{{ $presse->title }}" placeholder="Enter your title">
                </div>
                <div class="form-group">
                    ...
                    <select name="category" id="category" class="form-control bg-secondary">
                        ...
                        @foreach ( $presse_category as $category )
                            <option value="{{ $category->id }}" {{ $presse->category_id==$category->id ? "selected":"" }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="#">{{ get_phrase('Tags') }}</label>
                    <input type="text" name="tag" value="{{ $presse->tag }}" class="form-control bg-secondary" placeholder="Enter your tags">
                </div>
                <div class="form-group">
                    <label for="#">{{ get_phrase('Description') }}</label>
                    <textarea name="description" id="description" class="content" placeholder="Description">{{ $presse->description }}</textarea>
                </div>
                <div class="form-group">
                    <label for="">{{ get_phrase('Previous Uploaded Image') }}</label> <br>
                    <img src="{{ get_presse_image($presse->thumbnail,'thumbnail') }}" class="w-60 custome-height-50"  alt="">
                </div>
                <div class="form-group">
