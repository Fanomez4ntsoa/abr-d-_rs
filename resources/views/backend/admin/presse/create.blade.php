        ...
        <div class="col-12">
          ...
          >
            <div class="d-flex flex-column">
              <h4>{{ get_phrase('Add a new Article') }}</h4>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="eSection-wrap-2">
            <div class="eForm-layouts">
              @if ($errors->any())
                ...
                @endif
              <form method="POST" action="{{ route('admin.presse.created') }}" enctype="multipart/form-data">
                  @csrf
                  <div class="mb-3">
                    <label for="title" class="form-label eForm-label">{{ get_phrase('Article title') }}</label>
                    <input type="text" class="form-control eForm-control" id="title" name="title" placeholder="Page title">
                  </div>

                  <div class="mb-3">
                      <label for="presse_category" class="form-label eForm-label">{{ get_phrase('Select a category') }}</label>
                      <select name="category" class="form-select eForm-control select2" required>
                        <option>{{get_phrase('Select a category')}}</option>
                        @foreach(DB::table('presscategories')->get() as $category)
                  ...
                  <div class="mb-3">
                      <label for="description" class="form-label eForm-label">{{ get_phrase('Article details') }}</label>
